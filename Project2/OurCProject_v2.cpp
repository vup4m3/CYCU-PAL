...
enum Instr {
  ...
  LOAD_ADDR,
  // * Load Constant than push it in to stack 559
  LOAD_CONS,
  // * just the names of the (global) variables
  LIST_ALL_VAR,
  // * just the names of the (user-defined) 561
  LIST_ALL_FUNC,
  // * the definition of a particular variable, arg pop from Stack
  LIST_VAR,
  // * the definition of a particular function, arg pop from Stack 563
  LIST_FUNC,
  // * 564
  DONE,
  // * Call Func 565
...
public:
  ...
  bool Define_Func( int type, string name, vector<VarType> type_list, vector<Token> func_code ) {
    ...
    for ( i = mFunc_Table.begin() ; i != mFunc_Table.end() ; i++ ) {
      if ( i->Name() == name ) {
        *i = id;
        return true;
      } // if
